    ...
    try {
        ...
        while (keepFetching) {
            ...
            for (const repo of repos) {
                ...
                const sections = getSections(censoredReadme.split(/\r?\n/));
                ...
                for (const vbSection of voicebankSections) {
                    ...
                    matchedReleases.sort((a, b) => parseInt(b.name.replace(/\D/g, "") || 0) - parseInt(a.name.replace(/\D/g, "") || 0));

                    let lastRelease = matchedReleases[0];
                    if (!lastRelease && voicebankSections.length === 1) {
                        lastRelease = [...releases].sort((a, b) => parseInt(b.name.replace(/\D/g, "") || 0) - parseInt(a.name.replace(/\D/g, "") || 0))[0];
                    }
